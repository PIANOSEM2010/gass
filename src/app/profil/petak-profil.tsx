  ...
  return (
    <>
      ...
      {petak.length === 0 ? (
        ...
        <div className="grid grid-cols-3 gap-2 mt-3">
          {petak.map((p) => (
            <Link key={p.id} href={`/umpan/${p.id}`}
              className="rounded-xl border border-white/8 bg-[var(--kartu)] p-1.5 relative overflow-hidden">
        ...
        <div className="mt-3 rounded-2xl border border-white/8 bg-[var(--kartu)] divide-y divide-white/5">
          {petak.map((p, i) => (
