...
export default async function Umpan() {
  ...
  return (
    <div className="min-h-screen bg-[#071310] pb-8">
      {/* Kepala aplikasi */}
      <div className="sticky top-0 z-[900] bg-[#071310]/95 backdrop-blur border-b border-lime-400/10">
        <div className="max-w-md mx-auto flex items-center gap-2.5 px-5 pt-4 pb-2">
          <span className="w-9 h-9 rounded-xl bg-lime-400 text-slate-950 flex items-center justify-center">
            <IkonGowes size={20} accent="#062014" />
          </span>
          <span className="display-title text-xl text-white">BUG</span>
        ...
        <div className="max-w-md mx-auto flex gap-5 px-5">
          {[
            ...
            <Link key={t.href} href={t.href}
              className={`py-2.5 display-title text-sm tracking-wide border-b-2 ${t.aktif ? "text-lime-300 border-lime-400" : "text-slate-500 border-transparent"}`}>
              {t.label.toUpperCase()}
            </Link>
          ))}
        </div>
